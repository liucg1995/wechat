...
class UploadHandler
{
    ...
    protected $error_messages = array(
        ...
        'post_max_size' => '上传的文件字节数超过了服务器端限制',
        'max_file_size' => '文件太大',
        'min_file_size' => '文件太小',
        'accept_file_types' => '文件格式不符合要求',
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $this->options = array(
            'script_url' => $this->get_full_url().'/media/upload',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/upload/media/',
            'upload_url' => $this->get_full_url().'/upload/media/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function get_upload_path($file_name = null, $version = null) {
        ...
        if (empty($version)) {
            ...
            // log_message('info', print_r($this->options, true));
    ...
    public function post($print_response = true) {
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
                ...
                $extension = preg_replace('/.*\//','',$file_name);
                $files[] = $this->handle_file_upload(
                    $upload['tmp_name'][$index],
                    str_random(10).time().'.'.$extension
                    ,
